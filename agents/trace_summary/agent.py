...
This compresses a trace that ``trace_shell_context`` would otherwise blow
context on, without the embedding cost of ``trace_rag``. A useful
comparator for "is RAG worth it vs. plain summarization?"
...
SYSTEM_PROMPT = (
    ...
    "The prior trace shows what shell commands are available in this "
    "environment (each function_call name maps to a shell command of the "
    "same name installed in ``/usr/local/bin`` with matching ``--flag value`` "
    "arguments). Use ``shell_exec`` to run them. Stop when the task's "
