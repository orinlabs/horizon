...
class TraceKeywordAgent(BaseAgent):
    ...
    async def run(
        ...
        trace_result, err = await resilient_exec(
            environment, f"cat {TRACE_PATH} 2>/dev/null || true"
        )
        if trace_result is None:
            raise RuntimeError(f"failed to read trace: {err}")
        ...
        await environment.exec(f"rm -f {TRACE_PATH}")

        registry = await load_environment_tool_registry(environment)
        if registry is None:
            raise RuntimeError("TraceKeywordAgent requires /tools/tools.json in the env.")
        env_tools = get_environment_tool_schemas(registry)
        all_tools = [TRACE_SEARCH_TOOL, *env_tools]
            ...
            for tc in tool_calls:
                ...
                if name == "trace_search":
                    ...
                    payload = await call_environment_tool(
                        environment, registry, name, args
                    )
