...
Retrieval
---------
The LLM sees ``trace_search(query, k=3)`` plus the environment-owned tools
published by ``/tools/tools.json``. It never receives a generic bash tool.
...
SYSTEM_PROMPT = (
    ...
    "  - Any tools published by the task environment (for example tools named "
    "like those in the prior-session trace). Call those directly to act on "
    "the current world.\n\n"
...
class TraceRagAgent(BaseAgent):
    ...
    async def run(
        ...
        if registry is None:
            raise RuntimeError("TraceRagAgent requires /tools/tools.json in the environment.")

        # ---- Ingest: read trace, chunk by day, embed each chunk ----
        cat_result, cat_error = await resilient_exec(
            environment, f"cat {TRACE_PATH} 2>/dev/null || true"
        )
        if cat_result is None:
            raise RuntimeError(
                f"failed to read trace file at {TRACE_PATH} after retries: {cat_error}"
            )
        lines = (cat_result.stdout or "").splitlines()
        ...
        # `trace_search`.
        await resilient_exec(environment, f"rm -f {TRACE_PATH}")
        ...
        tools_schema = [TRACE_SEARCH_TOOL] + get_environment_tool_schemas(registry)
